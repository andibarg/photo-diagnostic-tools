...
class picture:
    ...
    '''
    def focus_peak(self,radius=1,amount=500,threshold=4000):
...
if __name__ == "__main__":
    ...
    ax[0,0].axis("off")
    ax[0,0].set_title("Focus peaking")

    ax[0,1].imshow(pic.img)
    ax[0,1].imshow(clip,cmap=plt.cm.bwr)
    ax[0,1].axis("off")
    ax[0,1].set_title("Highlight clipping")

    ax[1,0].imshow(pic.img)
    ax[1,0].plot(xgrid, ygrid,color='k',linewidth=1)
    ax[1,0].axis("off")
    ax[1,0].set_title("Grid lines")
    ...
    ax[1,1].set_ylim(0,np.mean(np.minimum.reduce(hists))*10)
